...
export function classifyRelevance(title, rawItem = {}) {
  ...
  // 2. DIRECT (옥외광고 직접 관련: 간판, 전광판, 학교간판, 아파트안내판, 승강기광고 등)
  const directRegex = /간판|옥외광고|지주간판|돌출간판|채널간판|LED\s*전광판|전광판|사이니지|전자게시대|미디어월|현수막|가로등배너|지정게시대|안내판|안내도|교통표지판|사인물|안내시스템|상징조형물|조형물|아치조형물|사인탑|조명탑|교표|교훈판|학교간판|교실표찰|전자현수막|강당전광판|체육관전광판|교내안내판|교문명판|교실안내도|승강기광고|엘리베이터TV|미디어보드|타운보드|단지안내판|아파트간판|동호수표찰|아파트게시판|동대표게시판|단지표지판|승강기모니터|단지사인물/;
  if (directRegex.test(title)) {
    return { tier: 'DIRECT', reason: '간판·전광판·조형물·안내판·현수막·학교사인·아파트사인 직접 관련' };
  }

  // 3. ADJACENT (인접 업종 후보군: 전시부스, 홍보관, 달력, 인쇄물, 졸업앨범 등)
  const adjacentRegex = /부스|홍보관|전시관|전시장치|독립부스|엑스포|인쇄|달력|수첩|리플릿|리플렛|포스터|문답지|에세이|졸업앨범|학교요람/;
  if (adjacentRegex.test(title)) {
    return { tier: 'ADJACENT', reason: '전시·부스·인쇄물·졸업앨범 등 인접 업종' };
  }
